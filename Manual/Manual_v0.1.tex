...
\fancyhead[LO]{Version 0.1 \\ December 22, 2015}
...
\par Note that if you plan to run many analyses in parallel, you should use a
\textbf{single-threaded} version of HyPhy.
...
\subsection*{A Note on Parallel Execution}
\par \BM is designed to run all predictions in a single thread. There is a joke
in here somewhere about Python programs and lack of concurrency \ldots For now,
all functions and supporting scripts are written for single-thread execution,
and parallel execution can be done with a tool like
\href{http://www.gnu.org/software/parallel/}{GNU Parallel}. Native parallel
