...
This section is specific to using `BAD_Mutations` on the [University of Minnesota Super Computing Institute](http://msi.umn.edu/) cluster. Our cluster uses the `module` command to add and remove certain programs from the user’s environment. The following commands should be run for `BAD_Mutations` on the cluster:
